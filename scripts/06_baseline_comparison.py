...
FEATURE_COLUMNS = ["Cs", "cover_mm", "D28", "m_aging", "time_idx", "t_year"]
...
def load_labeled_data(path: Path) -> pd.DataFrame:
    ...
    missing = [col for col in [*FEATURE_COLUMNS, "target_onset", "series_id"] if col not in df.columns]
    if missing:
...
def train_logistic_baseline(
    ...
    model.fit(train_df[FEATURE_COLUMNS], train_df["target_onset"])

    baseline = df[["t_year", "target_onset"]].copy()
    baseline["p_onset1_pred_baseline"] = model.predict_proba(df[FEATURE_COLUMNS])[:, 1]

    pf = (
        baseline.groupby("t_year", as_index=False)
        .agg(
            Pf_true=("target_onset", "mean"),
